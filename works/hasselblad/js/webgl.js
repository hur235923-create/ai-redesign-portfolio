...
function initGL(OGL) {
  ...
  const fragment = /* glsl */ `
    ...
    void main() {
      ...
      // "Light defines legacy" — 포인터·스크롤을 따라 카메라 위를 훑는 라이트 레이크
      vec2 lc = vec2(0.5) + (uMouse - 0.5) * 0.55;
      lc.y += (uScroll - 0.5) * 0.45;
      float ld = distance(vUv, lc);
      float light = smoothstep(0.6, 0.0, ld) * (0.12 + 0.05 * sin(uTime * 0.5) + uScroll * 0.06);
      col += vec3(0.98, 0.94, 0.86) * light * (1.0 + 0.6 * uHover);
