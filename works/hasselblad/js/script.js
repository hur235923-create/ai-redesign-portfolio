...
const hb_preSeen = sessionStorage.getItem("hb_preloaded") === "1";
if (hb_reduce || !hb_pre || hb_preSeen) {
  if (hb_pre) hb_pre.remove();
  hb_playHero();
} else {
  sessionStorage.setItem("hb_preloaded", "1");
  hb_lenis.stop();
...
if (!hb_reduce) {
  ...
  /* 라이트 스윕: 헤딩을 빛줄기가 훑으며 통과 (빛의 여정 모티프) */
  gsap.utils.toArray(".sweep").forEach((el) => {
    gsap.fromTo(el,
      { "--sweep": "-20%" },
      { "--sweep": "120%", ease: "none",
...
/* ---------- PRODUCTS 가로 핀 쇼케이스 (리빌드 가능) ---------- */
let hb_showTween = null;
function hb_buildShowcase() {
  const canH =
    !window.matchMedia("(prefers-reduced-motion: reduce)").matches &&
    !window.matchMedia("(max-width: 720px)").matches;
  const track = document.getElementById("showcaseTrack");
  if (hb_showTween) {
    if (hb_showTween.scrollTrigger) hb_showTween.scrollTrigger.kill();
    hb_showTween.kill();
    hb_showTween = null;
  }
  if (!track || !canH) {
...
if (hb_hoverable) {
  ...
  const hb_labels = [
    ...
    { sel: ".showcase", text: hb_horizontal ? "SCROLL" : "" },
  ];
