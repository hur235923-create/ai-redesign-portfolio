...
let ym_pulse = 0; /* 사운드 링 맥동(울림) — 스크롤 속도·클릭에 반응, 매 프레임 감쇠 */
ym_lenis.on("scroll", ({ scroll, velocity }) => {
  ...
  ym_pulse = Math.min(0.16, ym_pulse + Math.abs(velocity || 0) * 0.003);
});
/* 클릭 = 현을 튕기듯 링을 튕긴다 */
window.addEventListener("pointerdown", () => { ym_pulse = Math.min(0.24, ym_pulse + 0.13); });
...
gsap.utils.toArray(".count").forEach((el) => {
  ...
  el.setAttribute("aria-label", to + suffix); /* 스크린리더는 애니메이션 중간값이 아닌 최종값을 읽음 */
...
function ym_initRing() {
  ...
  if (mobile) { matcapMat.transparent = true; matcapMat.opacity = 0.42; } /* 모바일: 히어로 본문 가독성 위해 더 은은하게 */
  ...
  function render() {
    if (document.hidden) return; /* 탭 비활성 시 GPU 렌더 정지 */
    const t = clock.getElapsedTime();
    mx += (mouse.x - mx) * 0.05;
    my += (mouse.y - my) * 0.05;
    group.position.x = st.x + mx * 0.5;
    group.position.y = st.y - my * 0.4;
    group.scale.setScalar(st.scale * (mobile ? 0.78 : 1) * (1 + ym_pulse));
    ym_pulse *= 0.9;
